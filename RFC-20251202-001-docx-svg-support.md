## RFC-20251202-001: SVG Support in Forked `docx` Library for MDView
...
Current MDView DOCX export flows rely on the `docx` (dolanmiu/docx) library, which only supports raster image formats (PNG, JPEG, GIF, etc.). This limitation forces all vector content, including Mermaid diagrams, to be converted to PNG prior to embedding in DOCX. In browser-constrained environments (such as the MDView Chrome extension), this rasterization is both technically fragile (canvas taint rules, WASM loading constraints) and lossy (loss of scalability and sharpness).
...
- **Functional**:
  ...
  - Existing PNG/JPEG usages in MDView and other consumers behave identically to pre-fork behaviour.
...
- **Adoptability**:
  - MDView and other consumers can switch from a rasterization pipeline to direct SVG embedding with only local code changes (no fork-specific hacks beyond using the new API).
...
- **Goal 3**: Validate the approach through end-to-end tests and real-world integration (MDView exports with Mermaid diagrams, and at least one other document workflow).
...
```mermaid
graph TB
    A[User Code\n(MDView and other consumers)] --> B[ImageRun API\n(new SVG options)]
...
- **Metrics to observe in consumers** (MDView and others):
...
1. **Pre-migration preparation**:
   - Identify services currently using upstream `docx` for DOCX generation (MDView and others).
